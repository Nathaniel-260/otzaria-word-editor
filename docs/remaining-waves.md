...
- **`format.cs`/`bCs`/`iCs`/`fontSizeCs`/`rFonts` הם הליבה העברית.** ב-OOXML
  ...
  **ושימו לב לפני שנכנסים לכאן:** נמדד ב-Word 365 ש-`w:rtl` אינו דגל כיוון
  אלא **מתג שמחליף את כל מודל פתירת העיצוב** — ריצה עברית שמצוירת David-Bold-18
  בלעדיו מצוירת Arial-12 לא-מודגש איתו, ועיצוב שמגיע מסגנון אינו ניתן למראה
  ברמת הריצה כלל. הטבלה ב-`docs/engine-gaps.md`, בסעיף „ייצוא — תו ניטרלי”.
  לכן הגל הזה אינו „להוסיף `w:rtl` ואת המראה”: כל נגיעה בו מחייבת מראה מלאה
  בכל תכונה ובכל רמה של היררכיית הסגנונות, וזה שטח שנפסל פעם אחת כבר.
...
„אוצריא” נשאה „חידוש”/„קושיא”/„תירוץ” כשלושה כפתורים מנוטרלים, והם ירדו
ממנה — הפקד לא נדרש, וגם אין למנוע פעולה שיוצרת סגנון פסקה בשם
([superdoc/docx-editor#3975](https://github.com/superdoc/docx-editor/issues/3975),
פתוח). היא עדיין הבית הטבעי אם נשתמש בזה בהמשך.
