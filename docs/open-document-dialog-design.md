...
.open-dialog {
  width: min(840px, 94vw);       /* היה 960; ראו את ההערה בקוד — הכרטיס ירד למסגרת אפס */
  block-size: min(760px, 88vh);  /* גובה קבוע, לא max-height; ראו §1.4 */
  display: flex;
...
│   └ .open-pane                flex: 0 0 auto; block-size: 100%   ← יעד הנחיתה, חלון אחד בדיוק
...
מרווח בין שני האזורים בגוף, ובין שני האזורים בתוך `.open-pane`: `16px`
(`gap` על `.open-body` ועל `.open-pane`) — כלומר התוצאה החזותית זהה למה
שהייתה כשהשלושה היו אחים.
...
### 1.4 שתי נחיתות באותו מסך

למסך יש שתי כניסות — „מסמך חדש” ו„פתח קובץ” — ועד כה שתיהן הגיעו לאותו
פיקסל: ראש הרצועה. מי שביקש לפתוח קובץ קיבל מסך של תבניות, וגלל בעצמו בכל
פעם. ה-prop `intent` (`'new'` / `'open'`) הוא כל ההבדל, והוא מכתיב שני דברים:
לאן הגוף נגלל, ולאן המיקוד הולך.

| `intent` | גלילה | מיקוד |
|---|---|---|
| `'new'` (ברירת מחדל) | 0 | כרטיס התבנית הראשון |
| `'open'` | ראש `.open-pane` בקו העליון של חלון הגלילה | „עיון בקבצים…” |

**`.open-pane` היא מה שמאפשר את זה.** נחיתה דורשת מרחק גלילה, ומסך בלי
מסמכים אחרונים הוא בדיוק המקום שבו המרחק הזה נעלם — התוכן קצר מהחלון, ואין
לאן לגלול. `block-size: 100%` נותן לפן גובה של חלון גלילה שלם **בדיוק**
(ולא „לפחות”, שהיה נותן לרשימה ארוכה למתוח אותו ולהעביר את הגלילה מהרשימה
אל הגוף — §1.2), ולכן:

- מה שנשאר לגלול בגוף הוא **בדיוק** רצועת התבניות, בכל מצב;
- הנחיתה של „פתח קובץ” זהה בין „יש עשרים אחרונים” ל„אין אף אחד”;
- ובנחיתה, תחתית הפן מתלכדת עם תחתית החלון — הריפוד (16px) הוא מה שנשאר
  מעליה ומתחתיה, ולכן הכפתור אינו נדבק לקצה.

ה-100% נפתר מול תיבת התוכן של `.open-body`, וזו הסיבה שהגובה של
`.open-dialog` הוא `block-size` ולא `max-height` — **בניגוד** למה שנקבע
ב§10 („הדיאלוג מתקצר במקום לשמור מקום למה שאין”). ההכרעה הוחלפה במודע:
דיאלוג שגובהו נגזר ממספר האחרונים משנה את מרחק הגלילה בכל פתיחה, כלומר
„פתח קובץ” היה נוחת במקום אחר אצל כל משתמש — ואצל מי שאין לו היסטוריה לא
היה נוחת בכלל. התמורה השנייה, שאינה קשורה לנחיתה: „סגור” יושב באותו פיקסל
בכל פתיחה במקום לנדוד עם אורך הרשימה.

הגלילה עצמה נעשית ב-`landOnOpenPane` — הפרש בין שני `getBoundingClientRect`,
פחות הריפוד, על מיכל אחד. לא `scrollIntoView`: הוא מגלגל גם את כל האבות,
והמודאל יושב בתוך מעטפת שאין סיבה להזיז.

**איפה זה נמדד.** המיקוד — `tests/component/open-document-dialog.test.ts`
ו-`tests/component/shortcuts-core.test.ts` (שני הקיצורים, דרך המעטפת
המלאה). הגלילה — `scripts/open-dialog-probe.mjs` בלבד, בדפדפן אמיתי ובשני
תרחישים (עם אחרונים ובלעדיהם); ל-jsdom אין פריסה, ו-`scrollTop` שלו הוא
תמיד אפס, כלומר בדיקה שהייתה נכתבת שם הייתה עוברת ירוקה על קוד שבור.
...
בלבד ולא השארת השם הנגיש הטבעי:** כשהכרטיס השני עוד נשא אזהרה, קורא מסך הקריא
אותו כ„ספר קודש — שני טורים, גוף בשני טורים שווים עם כותרת רצה, הטורים מצוירים
הפוך בעורך הקובץ נשמר נכון” — שם של עשרים מילה. עם הפיצול השם הוא „ספר קודש —
שני טורים”, וההערה מגיעה כתיאור, כלומר **לפני ההפעלה ואחרי השם**. האזהרה עצמה
ירדה מאז (superdoc 2.12.0 מצייר את הטור הראשון בימין), אבל הפיצול נשאר: הוא מה
שיחזיק גם את האזהרה הבאה, וגם היום הרמז הוא תיאור ולא חלק מהשם.
...
`note` של `two-column` היה **„הטורים מצוירים הפוך בעורך; הקובץ נשמר נכון”**,
לצד `rtlColumnNote` שהופיעה בפס המצב אחרי הפעולה. שתיהן ירדו עם המעבר
ל-superdoc 2.12.0, שמצייר את הטור הראשון בימין (`SD-4764`; המדידה
ב-`docs/engine-gaps.md`). אין כרגע תבנית עם `note`, והעמודה נשארת בטבלה כדי
שהאזהרה הבאה תדע לאן ללכת.
...
| מצב | התנהגות |
|---|---|
| `recents.length === 0` | תיבת החיפוש והמונה אינם מרונדרים כלל. במקום הרשימה: `SvgIcon name="folder" :size="28"` ב-`--color-on-surface-variant`, „עדיין אין מסמכים אחרונים” (1em/600), „מסמך שתפתח מכאן או מ„עיון בקבצים…” יופיע כאן.” (0.88em/variant). האזור ממלא את `.open-pane` ואינו מתקצר — הכלל הזה **הוחלף**, וההנמקה ב§1.4: הדיאלוג בגובה קבוע כדי ששתי הנחיתות יהיו יציבות. |
...
## 13. העץ — אחרונים, ספריית אוצריא, ספרים אישיים ותיקיות

הפן של „פתח קובץ” (`.open-pane`) קיבל עמודת ניווט מימין, כמו חלונית הניווט
בסייר הקבצים וב„פתח” של Word. „עיון בקבצים…” נשאר בראש הפן, בדיוק במקומו.

| צומת | מה הרשימה משמאל מציגה | מקור |
|---|---|---|
| אחרונים | הרשימה של §4, ללא שינוי | `storage` |
| ספריית אוצריא ← מדפים | תתי-מדפים, ואחריהם ספרי Word | `library.getTree({ types })` |
| ספרים אישיים | הספרים ש-`source === 'user'` | אותו עץ |
| התיקיות שלי ← תיקייה ← תת-תיקייה | תתי-תיקיות, ואחריהן קובצי docx/docm | `fs.listUserFolder` |

**מידות.** העמודה 224px, והרשימה `minmax(0, 1fr)`. החלון גדל מ-840 ל-920
(ראו `.open-dialog`), ונקודות המעבר של §1.3 לא זזו. מתחת ל-680px של חלון
העץ עובר מעל הרשימה, בגובה של עד 160px.

**שורה בעץ.** 28px לפחות. הזחה של 14px לרמה. סדר מימין: חץ פתיחה (12px), אייקון (16px),
שם (נחתך בקצה), ומונה ספרים בגלולה. הנבחר מקבל `--color-primary-subtle` ופס של 3px
ב-`--word-blue` בקצה ההתחלה — אותה שפה כמו הלשונית הפעילה ברצועה.

**שורה ברשימה.** 34px לפחות. אייקון מסמך ב-`--word-blue` ותיקייה ב-`--color-on-surface-variant`.
השם מוצג בלי `.docx` (`.docm` נשאר, כי הוא אומר שיש מאקרו), ושורה שנייה למחבר ולמיקום בעץ.
הגיל והגודל יושבים בקצה.

**מהירות — מה המשתמש מחכה לו.**

- העץ נטען מהמטמון (`open-dialog-library`) כבר בעלייה, והדיאלוג נפתח מלא. הרענון מאוצריא
  רץ פעם אחת בהפעלה, ברקע. „טוען…” מופיע רק בפעם הראשונה.
- פירוט תיקייה נשמר בזיכרון. חזרה אליה מציגה מיד את מה שנראה קודם, והרענון מסובב רק את כפתור הרענון.
- המקום שנבחר נזכר בין הפעלות.
- `check:open-tree` מעכב את `getTree` בשתי שניות, ובודק שהעץ מופיע לפניהן.

**מהירות — כמה לחיצות.** לחיצה אחת על ספר או על קובץ פותחת אותו, ואין שלב „בחר ואז פתח”.
הבחירה בעץ עוקבת אחרי המיקוד, כך שחץ למטה מחליף את הרשימה בלי Enter. החיפוש בספרייה עובר על כל הספרייה.
חיפוש ב„אחרונים” מציע גם עד חמישה ספרים מהספרייה, עם „הצג הכול”.

**מקלדת (RTL).** בעץ, ↑↓ בוחרים. ← פותח ונכנס, ו-→ סוגר ויוצא, לפי השיקוף של WAI-ARIA לעץ.
ברשימה, → או Backspace עולים רמה, ו-↑ מהשורה הראשונה חוזר לחיפוש.

**כשאוצריא ישנה.** `unknown_method` מסתיר את הענף ואינו מציג שגיאה: הספרייה נעלמת, ו„הוסף תיקייה”
מתחלף בהסבר. תיקייה שנעלמה מהדיסק נשארת ברשימה, עם „נסה שוב” ו„הסר מהרשימה”, כי כונן שנותק יחזור.

**כפילות שנמנעה.** אוצריא מציבה את הספרים האישיים במדף משלה בעץ הספרייה. בלי טיפול הם הופיעו
פעמיים, כמדף וכצומת, בשני שמות זהים. `withoutPersonalShelves` מוריד מהספרייה מדף שכולו אישי,
ומדף מעורב נשאר כמות שהוא.
