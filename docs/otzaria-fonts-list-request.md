...
| | |
|---|---|
| ענף באוצריא | `feat/plugin-fonts-resolve-families` (טרם מוזג) |
...
**ההרשאה היא ההבדל המשמעותי מהמפרט המקורי**, שהניח מתודה חסרת הרשאה. התוסף
מצהיר `app.info.read` ב-`public/manifest.json`. בלעדיה `tryCall` מקבל סירוב,
מחזיר `null`, והמנייה נופלת לשכבת המדידה — כלומר הרשימה מתכווצת בשקט מ-294
לכמה עשרות. זו התסמונת שצריך לחשוד בה אם הבורר נראה דל.
