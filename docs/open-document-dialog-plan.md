...
2. **„שני טורים” נכתב כ-`w:cols` אמיתי.** ההחלטה המקורית כללה אזהרה בכרטיס,
   מפני שהמנוע מילא טורים שמאל→ימין גם ב-`w:bidi`. הפער נסגר ב-superdoc
   2.12.0 (`SD-4764`) ונמדד מחדש ב-`scripts/qa/column-selection-probe.mjs`:
   הטור הראשון נוחת בימין. האזהרה הוסרה מהכרטיס ומפס המצב — ראו
   `docs/engine-gaps.md`.
...
export interface DocumentTemplate {
  ...
  label: string;
  hint: string;
  note?: string;         // אזהרה בכרטיס. אין כרגע תבנית שנושאת אחת
