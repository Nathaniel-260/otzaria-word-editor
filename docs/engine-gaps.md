...
| `Shift+חץ מטה` בתחתית הטור | אינו חוצה את הגבול — 6 הקשות סימנו 4..23 במקום להגיע ל-26 |
...
**מה שנבדק ונשלל כפתרון בצד התוסף:** היפוך של `widths` ב-`setColumns` אינו
עוזר, מפני שהוא משנה את מה שנכתב ל-`w:cols` — כלומר שובר את הייצוא שהיום
נכון — ואינו נוגע בסדר המילוי, שהוא החלטה של הפריסה.
