...
2. `scripts/qa/bold-cs-qa.mjs` רץ עם `repairComplexScriptBold` **מושבת**, ושלוש
   שורות המסך עוברות: הקובץ שדווח, `bCs` על הריצה, ו-`bCs` על ברירות המחדל.
...
| גרירה בסדר ה**קריאה** (ימין ואז שמאלה) | הטור הימני הגיע ל-8 שורות מסומנות וירד ל-5 אחרי המעבר; בסוף 20..30 (11) |
| סמן ברוחב הטור הימני מתחת לשורה האחרונה שבו | הראש אינו מהודק לסוף הטור — 6 מלבנים משמאל ו-5 מימין |
| `Shift+חץ מטה` בתחתית הטור | לא רק שאינו חוצה — הוא **נע אחורה**: סמן מאומת בשורה 23, ושש הקשות **מטה** נתנו 4..23, כלומר הראש עלה 19 שורות |
...
שמתחילה בו וממשיכה שמאלה הולכת **אחורה** בסדר המסמך: היא אינה מרחיבה את
הבחירה אלא מכווצת אותה, ומה שכבר סומן בימין נמחק.
