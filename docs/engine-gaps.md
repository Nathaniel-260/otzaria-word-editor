...
נמדד ב-superdoc 2.10.0 (engine 0.9.0), Chrome אמיתי על ה-dist הארוז, על הקובץ
שדווח (סגנונות „כותרת 2/3/4/5” נושאים `<w:bCs/>` ואין בהם `<w:b/>`):
...
**מה נשלח:** ההשלמה נעשית ב-`src/engine/docx-preflight.ts`, לפני שהמנוע רואה את
...
…)”. `resolveRunScriptContext` אינו קיים בשום קובץ, וגם לא ב-2.11.0-next.9.

תיקון נכתב אצלנו על מקור המנוע (`C:/tmp/superdoc-src`, ענף
`fix/complex-script-bold`): בחירת מחסנית לפי `w:rtl`/`w:cs` עם נפילה חזרה
לדגל הלטיני, ובדיקות.
