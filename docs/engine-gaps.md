...
השלישי כאן — עצירת טאב שזזה עם ההזחה — נמצא אחריהם, כשנוספה ההזחה לפי רמה.
...
עם `border-bottom: 1px dotted`. העצירה גם שורדת `toc.update`. העלות נמדדה:
26ms לשורה בממוצע (10 שורות ב-261ms).
...
הלוגי, ובפסקה עברית הכותרת נדחפת מימין פנימה. מה שנשבר הוא רק עמודת המספרים,
ולכן `src/engine/toc.ts` נסוג עם העצירה באותו שיעור בדיוק.

**מה זה עולה בצד השני:** קובץ שנפתח ב-Word יראה את מספרי רמות 2 ו-3 מוסטים
ב-220 ו-440 twips (‏4 ו-8 מ"מ) מעמודת רמה 1, מפני ש-Word אינו מזיז את
העצירה מלכתחילה. זו אותה הכרעה כמו בקיזוז שלמעלה: מה שנראה בעורך נכון, והסטייה
— בגודל הזה — נשארת בצד השני. שני הפיצויים ייעלמו יחד ביום שהמנוע ימדוד כמו
Word.
...
**נעשה מאז:** `layOutTocRows` כותבת את העצירה לכל שורה, והפער אושרר מחדש על
2.13.0 ודווח כ-[#3988](https://github.com/superdoc/docx-editor/issues/3988).
הפירוט המלא — כולל ההזחה לפי רמה ושני הקיזוזים — בסעיף „תוכן עניינים — שני
פערים שהתגלו מדיווח משתמש” בראש הקובץ.
