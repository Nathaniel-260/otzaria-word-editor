...
בחירת העוגן נעשית היום בשני צעדים, מפני שהעוגן הראשון נשלח תמיד כ-
`nodeType: 'paragraph'` — גם כשהבלוק שלפני הכיתוב הוא טבלה (ראו הסעיף הבא,
שבו ההנחה הזאת נמדדה כשגויה):
...
**והנפילה-לאחור הזאת מיותרת מאז שנמדד שהמנוע כן מקבל עוגן שאינו פסקה** (ראו
הסעיף הבא): היא **תוסר/תצומצם בקומיט נפרד**. מה שנשאר נכון בלי קשר לכך הוא
שהעוגן נקרא **לפני** ההסרה, ורשת השחזור שאחריה.
...
**ההנחה שנרשמה כאן נמדדה כשגויה — נמדד מחדש על 2.15.0-next.15 (15.9.2026),
ואותה תוצאה בדיוק על 2.14.0-next.5.** ‏`adjacentTo` **כן** מקבל בלוק שאינו
פסקה: הכתובת `{kind:'block', nodeType:'table', nodeId:'tbl:…'}` מתקבלת גם
עם `position: 'below'` וגם עם `'above'`, וסדר הבלוקים שיוצא הוא בדיוק
`פסקה │ כיתוב „טבלה 1: מעל ללוח” │ טבלה │ כיתוב „טבלה 2: מתחת ללוח” │ פסקה`.
מה שנדחה הוא הצורה ש**אנחנו** שולחים: `nodeType: 'paragraph'` עם מזהה של
טבלה — `TARGET_NOT_FOUND` („target paragraph tbl:… was not found”), וזו גם
ההודעה שנרשמה כאן במדידה הראשונה. כלומר נמדדה טעות שלנו, לא מגבלת מנוע,
והמנוע לא השתנה ב-2.15.

**מה נגזר:** העקיפה — בחירת עוגן בשני צעדים, והסירוב כששני השכנים אינם
פסקאות — **תוסר/תצומצם בקומיט נפרד**; מה שכן נשאר נחוץ הוא קריאת ה-`nodeType`
מ-`blocks.list`, שהיא בדיוק מה שמאפשר להעביר את הסוג האמיתי. סוגי בלוק
אחרים (תוכן עניינים, `sdt`) לא נמדדו, ולכן הרחבה מעבר לטבלה דורשת מדידה
משלה. „נקי” בסעיף הכיתובים שלמטה מתייחס לקוד השדה שנכתב, לא לקבילות העוגן.
