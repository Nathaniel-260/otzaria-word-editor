...
- **אין דרך ליצור מקטע סביב הבחירה — שלושה מסלולים, שלושתם סגורים.** זה מה
  ...
  סביב הבחירה, ו-`w:cols` על מה שביניהם. נמדד על 2.14.0-next.5:
...
הרמות כדי שה-DOCX יישאר תקין גם ב-Word. הפער אושרר מחדש על 2.13.0 ודווח
...
ו-33 פסקאות. הגשש: `scripts/spacing-preview-probe.mjs`.
...
ו-`Ctrl+End` נמדדו תקינים, וכך גם החצים בעברית (`ArrowRight` מקדם אחורה
לוגית). כלומר תנועת הסמן היא מודעת-כיוון בכל מקום **חוץ** מ-`End`.
