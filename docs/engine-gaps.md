...
השער: `scripts/qa/bold-cs-qa.mjs`. הוא אינו ריק — עם התיקון מושבת בבאנדל, שלוש
השורות נופלות (400), ובקרה של „טקסט בלי `bCs` נשאר דק” עוברת בשני המצבים.
...
…)”. `resolveRunScriptContext` אינו קיים בשום קובץ, וגם לא ב-0.10.0 שיצא מאז.
...
**לא נעקף כאן.** התנאי המדויק ידוע עכשיו — „הסמן בתוך העמוד ולא מעל
`.superdoc-line`" — ולכן עקיפה כבר אינה חייבת להיות היוריסטיקה: אפשר לחסום
מהמנוע בדיוק את אירועי התנועה שנופלים ברצועה המתה, והבחירה תיעצר בערך האחרון
התקין במקום להימחק, בדיוק כפי שהיא נעצרת כשיוצאים מהעמוד. נדרש קודם למדוד
באיזה אירוע המנוע משתמש (mousemove מול pointermove) ועל איזה צומת הוא מאזין.
...
## טורים ב-RTL — הנזק אינו בתצוגה אלא בבחירה

### סדר מילוי הטורים קבוע שמאל→ימין, ואינו קורא `w:bidi`

`docs/superdoc-2.10-review.md` מדד שהמנוע ממלא טורים משמאל לימין גם במקטע
`w:bidi`, וסיכם ש„הנזק בתצוגה בלבד” מפני שה-docx המיוצא יוצא שלם. **הסיכום
הזה מיושן.** נמדד שוב על superdoc 2.11.0 / docx-engine 0.10.0, עם גרירה
אמיתית (`Input.dispatchMouseEvent`) והבחירה נקראת מהמנוע ולא מוסקת ממיקום
הטורים — `scripts/qa/column-selection-probe.mjs`:

| מה נבדק | מה נמדד |
|---|---|
| הייצוא | נקי — `<w:cols w:num="2"/>` ו-`w:bidi` ב-`sectPr` |
| שורה 01 | נוחתת בטור **השמאלי** (x=538, אמצע הדף 700) — בוורד היא מתחילה מימין |
| גרירה בסדר המסמך | רציפה וחוצה את הגבול — 6..30 |
| גרירה בסדר הקריאה (מהטור הימני שמאלה) | הבחירה **מתהפכת סביב העוגן**: 20..30 במקום להמשיך מ-30 |
| סמן מתחת לשורה האחרונה בטור הימני | הראש אינו מהודק לסוף הטור |
| `Shift+חץ מטה` בתחתית הטור הראשון | **גולש לתחילת המסמך** — העוגן ב-23, ו-6 הקשות נותנות 4..23 |

הבחירה נשארת **רצף אחד** (`contiguous: true`), ולכן זו אינה מחיקה אלא היפוך:
בעברית הטור שנקרא ראשון הוא הימני, והוא ה*שני* בסדר המסמך. כלומר הפגם נפגש
בכל גרירה טבעית, ולא רק בעין. אין תיקון בצד שלנו: סדר המילוי הוא של המנוע,
והוא בקוד סגור. מה שכן בידינו — הודעה למשתמש ב-`applyColumns`
(`src/engine/page-setup.ts`) — אינו קיים כיום, וזו ההמלצה שהגשש רושם.
