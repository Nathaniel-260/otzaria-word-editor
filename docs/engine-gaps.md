...
## שני טורים — סדר הטורים נסגר ב-2.12.0; מה שנשאר אינו תלוי בכיוון
...
מסמך, וזו השורה שמכריעה מה שייך ל-`w:bidi` ומה לטורים בכלל:
...
| `Shift+חץ מטה` בגבול שבין הטורים | **גולש לתחילת המסמך**: עוגן שאומת בשורה 23, שש הקשות מטה → 4..23 | **זהה**: 4..23 |
...
2. **מה שנשאר שבור אינו פער של RTL.** ‏`Shift+חץ מטה` בגבול שבין הטורים נותן
   **בדיוק את אותה תוצאה** במקטע לועזי ובמקטע עברי: הראש עולה 19 שורות אחורה
   במקום להגיע לראש הטור השני. פער שאינו תלוי בכיוון אינו יכול להיתלות
   ב-`w:bidi`, ואינו מצדיק הודעה שמדברת על מקטע עברי.
...
והגרירה סביב הטורים; מיקום התיקון של `Shift+חץ מטה` הוא שם ולא אצלנו — ניווט
הסמן האנכי אינו נגזר משום קלט שה-Document API חושף.
