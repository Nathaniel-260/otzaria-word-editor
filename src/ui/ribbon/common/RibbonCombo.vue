  ...
  >
    ...
    >
      ...
      <!--
        ...
        ה-`<span>` הפנימי הוא מה שמאפשר את שתי השורות במצב הדגימה: הקטיעה שם
...
/**
 * מצב הדגימה: הפס מציג משפט קבוע ולא טקסט של המשתמש, ולכן שתי שורות בגודל
...
/*
  ...
  **בלי בחירה** (`specimen`) — „אילו אותיות יש בגופן הזה”. אין טקסט של המשתמש
  ...
  בגודל הקבוע של הרשימה, ובשתי שורות. ראו למטה.
*/
.ribbon-combo-sample {
  position: sticky;
  inset-block-end: 0;
...
/*
  מצב הדגימה — הפסוק, בשתי שורות.

  ## למה שתיים, וזו מדידה ולא טעם
  ...
  האותיות שלא נבדקו. שתי שורות מכסות את הפסוק בכל גופני הטקסט שנמדדו; גופן
  רוחב-קבוע כמו Courier New נקטע אחרי שתיים, כפי שנקטע גם קודם.

  ## ולמה `line-height: 20px` דווקא

  30px הוא של מצב הבחירה, שם הוא ממרכז שורה אחת בגודל שמגיע ל-24px. שתי שורות
  בו היו 60px, ובגובה 56 היו נחתכות. 20px מכיל את 14px של הדגימה בכל גופן
  שנמדד (תיבת השורה הטבעית הגיעה ל-19px ב-Segoe UI וב-Levenim MT), ושתי שורות
  יוצאות 40px — בתוך 43 של תיבת התוכן, עם אותו מרווח אמיתי שגובה 44 משאיר
  לשורה אחת.

  ‏`line-clamp` הוא מה שמונע שורה שלישית לגלוש מתחת לגבול, ומוסיף את שלוש
...
/* שתי שורות של 20px, ריפוד ומסגרת — ועוד 3 של מרווח, כמו במצב הבחירה. */
