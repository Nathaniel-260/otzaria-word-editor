<template>
  ...
  >
    ...
    <ul
      ...
      role="listbox"
      :style="[popoverStyle, { minWidth: listMinWidth, width: listWidth }]"
...
const props = withDefaults(
  defineProps<{
    ...
    listMinWidth?: string;
    /**
     * רוחב **קבוע** לרשימה, במקום „רחב כמו השורה הרחבה ביותר”. אופציונלי:
     * בלעדיו נשאר `width: max-content` שבגיליון.
     *
     * ## למה זה קיים, ולמה דווקא בבורר הגופן
     *
     * `max-content` מחייב את הדפדפן למצוא מי השורה הרחבה ביותר, וכל שורה בבורר
     * הגופן מצוירת **בגופן של עצמה**. כלומר כדי לענות על השאלה הזאת הוא חייב
     * לטעון את כל הגופנים שברשימה — גם של שורות שמחוץ למסך.
     *
     * נמדד על 251 שורות (המכונה מנתה 287 משפחות): פתיחה ראשונה **2118ms**.
     * אותן שורות בדיוק, כשכולן באותו גופן אחד — **107ms**. הגלילה, לעומת זאת,
     * זהה בשני המקרים (33ms למסך), מפני שהגופנים כבר נטענו בפריסה.
     *
     * הגובה כבר קבוע (`height: 28px` למטה) וזה מה שהופך את הגלילה לזולה;
     * הרוחב הוא מה שנשאר.
     *
     * ומה זה עולה במראה: כלום. השורה הרחבה ביותר נמדדה **238px**, והתקרה
     * שכבר הייתה בגיליון היא 260px — כלומר רוחב קבוע יושב בתוך טווח שהרשימה
     * ממילא לא חרגה ממנו. שם ארוך מהרוחב מקבל גלילה אופקית, בדיוק כפי שקיבל
     * קודם כל שם שחרג מ-260px.
     *
     * **לא** ברירת מחדל: בורר הגדלים חייב להתכווץ לתוכן („8, 9, 10” אינו צריך
     * 248 פיקסלים), ורשימה קצרה של שמות ממילא אינה משלמת דבר על `max-content`.
     */
    listWidth?: string;
    /**
...
.ribbon-combo-list {
  ...
  */
  ...
  /*
    ...
    `max-content` הוא ברירת המחדל ולא הכלל היחיד: מי שמעביר `listWidth` דורס
    אותו ברוחב קבוע. ראו את ההסבר שם — בבורר הגופן `max-content` עולה 2118ms
    בפתיחה, מפני שהוא מחייב לטעון כל גופן ברשימה כדי למצוא את השורה הרחבה.
  */
  width: max-content;
