<template>
  <div class="ribbon-tab-pane file-tab">
    ...
    <!--
      ...
      **סדר הפקדים בקבוצה הפוך לדפוס של „לוח”,** ובכוונה. שם הפקד הגדול
      („הדבק”) הוא הפעולה הראשית והמחסנית משרתת אותו; כאן אין פעולה ראשית —
      „אודות” אינו הכותרת של „קיצורים”, ושני הקיצורים אינם וריאנטים שלו. מה
      שכן קיים הוא זיקה בין השניים: הרשימה והניהול שלה הם שתי פעולות על אותו
      דבר, ולכן הם המחסנית, ו„אודות” יורד לקצה השמאלי כפקד נפרד.

      „קיצורים” ירד מגדול לקטן במחסנית ולא בכוונה להצניע אותו: הוא נשאר בשורה
      הראשונה של המחסנית, כלומר בגובה העין, וה-tooltip שלו עדיין מציג את
      הצירוף מהרג'יסטרי. מה שהקבוצה קנתה בתמורה הוא ש„ניהול קיצורים” גלוי
      לצידו — דיאלוג שמגיעים אליו רק במקרה הוא דיאלוג שאיש לא ימצא, וזה בדיוק
      מה שנימק את הכפתור של „קיצורים” מלכתחילה.
    -->
    <RibbonGroup
      title="מידע"
      icon="info"
    ...
    >
      <RibbonStack>
        <!-- הוא **כל הרשימה** של הקיצורים, כלומר בדיוק מה שמי שאינו יודע
             אותם מחפש. ה-tooltip מציג את הצירוף מהרג'יסטרי, ולכן הכפתור גם
             מלמד אותו. -->
        <RibbonButton
          icon="book"
          label="קיצורים"
        ...
        />
        <!-- אין לו `shortcut-id`, ואין לו צירוף ברג'יסטרי: כל צירוף שנתפס
             כאן הוא צירוף שנגרע מהמשתמש, וזה בדיוק מה שהדיאלוג הזה נועד
             לחלק. הוא נמצא מהרצועה ומ„ספר לי”. -->
        <RibbonButton
          icon="keyboard"
          label="ניהול קיצורים"
          variant="small"
          tooltip="הצמדת צירוף מקשים לערכת עיצוב"
          @click="$emit('shortcuts-manage')"
        />
      </RibbonStack>
      <!-- הפקד היחיד בלשונית בלי `:disabled`, ובכוונה: הדיאלוג הוא של התוסף,
           ...
           שני דיאלוגי הקיצורים אינם מקבלים `:disabled` מאותו טעם בדיוק:
           רשימת הקיצורים וניהולם אינם נוגעים במסמך. הצמדת עיצוב **כן** דורשת
           מסמך, וזה נבדק בזמן הלחיצה על הצירוף — לא כאן. -->
      <RibbonButton
        icon="info"
        label="אודות"
        variant="large"
