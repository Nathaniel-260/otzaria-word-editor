 ...
 *    נופלת לשכבה הבאה.
 *
 *    **דורשת את ההרשאה `app.info.read`**, שמוצהרת ב-`public/manifest.json`.
 *    הרשאה חסרה אינה שגיאה שרואים: הסירוב חוזר כ-`null`, המנייה נופלת לשכבה 2,
 *    והרשימה מתכווצת בשקט מ-294 משפחות (נמדד ב-Windows) לכמה עשרות. זו
 *    התסמונת שצריך לחשוד בה אם הבורר נראה דל פתאום.
...
/** ההרשאה ש-`fonts.listInstalled` דורשת. מוצהרת ב-`public/manifest.json`. */
export const FONTS_PERMISSION = 'app.info.read';

/**
 * למה המארח לא ענה — לקונסולה, פעם אחת.
 *
 * שני מצבים שנראים זהים מבחוץ ודורשים תיקון שונה לגמרי: אוצריא ישנה שאינה
 * מכירה את המתודה, והרשאה שלא אושרה. שניהם חוזרים כ-`null` מ-`tryCall`, ולכן
 * ההבחנה ביניהם דורשת לשאול מה אושר בפועל.
 *
    ...
    console.warn(
      `[otzaria-word] fonts.listInstalled לא החזירה רשימה — הבורר נופל למדידה מקורבת. ` +
        (granted === null
          ...
          : has
            ? `ההרשאה ${FONTS_PERMISSION} מאושרת, ולכן זו כנראה גרסת אוצריא שאינה מכירה את המתודה.`
...
/** כל כמה שמות מוותרים על החוט. */
const YIELD_EVERY = 40;

/**
 * מסננת למה שהדפדפן באמת פותר.
 *
 * מוותרת על החוט כל `YIELD_EVERY` שמות: מדידה של מאות משפחות היא סינכרונית
 * מטבעה, וללא הוויתור היא הייתה בולעת פריים שלם בזמן שהמשתמש מקליד.
 */
