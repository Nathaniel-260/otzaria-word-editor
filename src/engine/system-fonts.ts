 ...
 *    נופלת לשכבה הבאה.
 *
 *    **אינה זמינה עדיין:** אין ב-otzaria מתודה בשם הזה, והצד שלה מתועד ב-
 *    docs/otzaria-fonts-list-request.md כבקשה. עד שהיא תיכנס כל משתמש נופל
 *    לשכבה 2 — כלומר הרשימה היא כמה עשרות שמות ולא 294 (מה שנמדד ב-Windows על
 *    מימוש ניסיוני). אין כאן הצהרת הרשאה: `app.info.read`, שהמתודה תשב תחתיה,
 *    היא **הרשאת בסיס** שאוצריא מעניקה אוטומטית, והוולידטור שלה ממליץ להסיר
 *    הצהרות מיותרות ממניפסטים (ראו host/otzaria-client.ts).
