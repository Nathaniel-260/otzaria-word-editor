...
/**
 * „עמודות” — ובלי הודעה נלווית.
 *
 * עד superdoc 2.11.0 הפעולה הייתה מלווה ב-`rtlColumnNote`: המנוע מילא את
 * הטורים שמאל→ימין גם כשב-`sectPr` יש `w:bidi`, בעוד ECMA-376 §17.6.1 קובע
 * ש-`w:bidi` הוא שמכריע את סידור הטורים — כלומר שהעמודה הראשונה שייכת לצד
 * **ימין**. הפער נסגר במנוע (`SD-4764`, נכלל ב-2.12.0), ונמדד מחדש כאן:
 * ב-`scripts/qa/column-selection-probe.mjs` שורה 01 נוחתת בטור **הימני**,
 * וגרירה בסדר הקריאה מסמנת בדיוק את הטווח שהתבקש. לכן ההודעה הוסרה — הודעה
 * שמתארת התנהגות שכבר אינה קיימת גרועה מהיעדרה.
 *
 * מה שעדיין פתוח אינו RTL אלא טורים בכלל: Shift+חץ מטה בגבול שבין הטורים
 * גולש לתחילת המסמך, **זהה ב-LTR וב-RTL** (נמדד; ראו `docs/engine-gaps.md`).
 * פער שאינו תלוי בכיוון אינו שייך להודעה על מקטע עברי.
 */
