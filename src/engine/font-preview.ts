 ...
 *
 * ### וגם „לצבוע ואז לבטל בעצמנו” נמדד ונשלל — הוא הורג את ה-redo
 *
 * המסלול המתבקש אחרי כל אלה הוא `format.apply` בריחוף ו-`doc.history.undo()`
 * ביציאה. הביטול עצמו נקי להפליא — `document.xml` חוזר **זהה בייט-בבייט**,
 * `undoDepth` חוזר למקום, והבחירה נשמרת — ובכל זאת נשלל בשתי מדידות:
 *
 * 1. **הריחוף מלכלך פעמיים.** `markDirty` נמדד גם על הצביעה וגם על הביטול,
 *    שניהם סינכרוניים למוטציה. „צבע ובטל”, שהוא נטו אפס במסמך, עדיין מסמן
 *    „לא נשמר” ומתזמן autosave שכותב לקובץ גופן שאיש לא בחר.
 * 2. **ה-redo של המשתמש נהרס, גם כשהוא ביטל.** נמדד מקצה לקצה: משתמש עם
 *    `redoDepth: 1` ממתין (הקליד, ביטל) — הצביעה מוחקת אותו (1→0), והביטול
 *    שלנו מציב במקומו את **הגופן שלנו** (0→1). `Ctrl+Y` מחזיר `Courier New`
 *    במקום את ההקלדה. כלומר ריחוף שבוטל מחק עבודה שהמתינה ל-redo.
 *
 * לשנייה אין גידור: מחיקת מחסנית ה-redo היא ההתנהגות הנכונה של כל עורך על
 * מוטציה חדשה, ואין ב-superdoc 2 מוטציה שאינה נוגעת בהיסטוריה. זה הפער
 * שסוגר את המסלול, והוא חמור מן החיתוך שלמטה — חיתוך הוא כיעור, וזה אובדן
 * עבודה. הטבלה המלאה ב-`docs/engine-gaps.md`.
 *
 ...
 * המשתמש בגופן שמרחפים עליו **ובגודל שבמסמך** (`sampleSize` ב-
 * `use-font-controls`; גופן נראה אחרת לגמרי ב-11pt וב-20pt, ומי שבוחר כתב
 * לספר בוחר את הצירוף). הוא נאמן מפני שאין בו עימוד לשקר עליו, והוא נשען
