...
> **65 מתוך 76 האייקונים** ב־`icons.ts` הם Fluent System Icons. אחד־עשר היוצאים
> נשארים ציור בבית, ולא מאותה סיבה — וההבחנה חשובה כאן, כי הצהרת רישוי שמונה
> אייקון של הפרויקט בין המכוסים ב־MIT מרחיבה את מה שמוצהר מעבר למה שנלקח:
>
...
>   וריאנט `_rtl` כמו ש־`arrow_export` דווקא כן מציע. כלומר הפעם דווקא
>   *היעדר* וריאנט ה־RTL הוא מה שמונע את המעבר.
> - `header`, `footer`, `firstPageHeader`, `oddEvenPages` — כותרת עליונה,
>   כותרת תחתונה, „שונה בעמוד ראשון” ו„שונה בעמודים זוגיים ואי־זוגיים”.
>   ל־Fluent System Icons אין גליף לכותרת עליונה או תחתונה של מסמך בכלל, ולכן
>   הם מצוירים בבית בגריד ובמשקל של השאר.
> - `pageNumber`, `dateTime`, `updateFields`, `bookmark` — „מספר עמוד”,
>   „תאריך ושעה”, „עדכן שדות” ו„סימנייה”. הפעם הסיבה אינה היעדר גליף אלא
>   היעדר ה־path data עצמו: ההצהרה הזאת מכסה path data **מקורי**, כפי שהוא
>   בחבילה של Microsoft, ולא שחזור מהזיכרון של גליף דומה. שחזור כזה אינו הקוד
>   של Microsoft ואינו זכאי להצהרה עליו, ולכן ארבעתם צוירו בבית — על אותה
>   טבעת דף של ארבעת שכניהם, כדי ששורת הפקדים תישאר במשקל אחד.
