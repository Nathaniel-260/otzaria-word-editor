...
> „הפניות”), מ־`document_bullet_list`. התבליטים שלו יושבים אצל Microsoft
> משמאל לשורות — שורה שנקראת משמאל לימין — ובממשק עברי זה הפוך. ב־1.1.338 יש
> 19 גליפי `_rtl` בגריד 20, ואף אחד מהם אינו `document_*`; אבל הכלל שלהן נמדד
> מהחבילה: `clipboard_bullet_list_rtl` משקף את התבליטים בתוך הלוח ומשאיר את
> הלוח עצמו, ו־`text_bullet_list_rtl` הוא שיקוף אופקי מלא של `text_bullet_list`.
> כלומר מה שזז ב־RTL הוא התוכן, לא המכל.
>
> לכן ה־path של Microsoft **אינו משתנה בתו אחד**: הוא מפוצל לשניים לפי גבול
> הצורה — הדף (כולל קיפול הפינה) והתוכן — והתוכן עוטף ב־`<g transform>` שמשקף
> סביב מרכז הדף. השיקוף הוא קוד של התוסף; הקווים הם של Microsoft. `check:icons`
> מאמת בכל הרצה שהטענה הזאת נכונה — שרשור החלקים לפי סדר המקור חייב להחזיר את
> `document_bullet_list` בדיוק, כך שציור מחדש ייפול. ה־MIT מתיר שינוי ממילא;
> ההפרדה כאן היא כדי שההצהרה תישאר מדויקת לגבי **מה** נלקח.
>
