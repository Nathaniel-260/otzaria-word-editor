 ...
 * 2. **הקיבוץ קרה?** רשימה של מאות שמות בלי `<optgroup>` אינה שמישה.
...
try {
  ...
  */
  ...
  if (extra.length === 0) {
    report.skip('הקיבוץ ל-optgroup', 'אין מה לקבץ בלי מנייה');
  } else if (groups.includes(GROUP_HEBREW) || groups.includes(GROUP_ALL)) {
    report.pass('הקיבוץ ל-optgroup', groups.filter(Boolean).join(' / '));
  } else {
    report.fail('הקיבוץ ל-optgroup', `הרשימה שטוחה — ${names.length} שמות ברצף אחד`);
  }
