 ...
 * בסדר המסמך; לכן גרירה טבעית ממנו שמאלה מוחקת את מה שכבר סומן במקום להמשיך
 * אותו. זה הפנים שהמשתמש פוגש, וזה מה שנמדד כאן.
 *
 * ## מה נמדד
 *
 ...
 *   7. Shift+חץ מטה בגבול שבין הטורים — האם המקלדת היא מעקף. השורה מבחינה
 *      בין „לא חצה” לבין „נע אחורה”: השני חמור יותר, וכותרת אחת לשניהם הייתה
 *      מוכרת אותו בפחות ממה שנמדד.
 *
...
try {
...

  {
    ...
    if (!(await placeCaret(at(anchorLine)))) {
      ...
      // הראש **מעל** העוגן = הבחירה נעה אחורה. זה ממצא אחר מ„לא חצה”, וחמור
      // ממנו, ולכן הוא מקבל כותרת משלו ולא נבלע בזו של הגבול.
      const backwards = got.count > 0 && got.first < anchorLine;
      if (startsRight && crossed) {
        report.pass('Shift+חץ מטה חוצה את הגבול בין הטורים', got.label);
      } else {
        report.fail(
          backwards
            ? 'Shift+חץ מטה מזיז את ראש הבחירה אחורה במסמך, לא מטה'
          ...
          `6 הקשות מסמן בשורה ${anchorLine}: ${got.label} — ` +
            ...
            `${backwards ? ` (הראש עלה ${anchorLine - got.first} שורות אחורה)` : ''}, ` +
