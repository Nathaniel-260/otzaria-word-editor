 ...
 * 3. **מה שאין לגעת בו** — ריצה לטינית, ריצה מעורבת, קוד שדה, `rPr` של סימן
...
describe('סימון ריצות עבריות', () => {
  ...
  it('ריצה מעורבת אינה מסומנת — סימון היה מחיל גופן מורכב על הלטינית שבה', () => {
    const out = markRtlRuns(doc(para(run('שלום world'))));
    expect(out).toBeNull();
  });
